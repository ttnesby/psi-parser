...
fun KtClass.getRuleServiceFlow(bindingContext: BindingContext): Result<Sequence<FlowElement>> =
    getOverriddenProperty(RuleMethod.RULE_SERVICE).mapCatching { property ->
        property.streamRuleServiceElements(RuleSuperClass.RULE_FLOW, bindingContext).getOrThrow()
    }
...
/**
 * KDoc er enten et barn av PsiElementet eller ligger som et søsken-element umiddelbart før dette elementet.
 * Det lages en sekvens som starter fra forrige søsken-element og fortsetter til forrige søsken-element for hver iterasjon.
 * Filtrerer ut PsiWhiteSpace og KDoc-elementer og sekvensen stopper når et element er hverken KDoc eller PsiWhiteSpace.
 */
...
private fun KtCallExpression.extractGrener(): List<FlowElement.Gren> {
    return getLambdaBlock()
        .map { block ->
            block.statements.map { gren ->
                FlowElement.Gren(
                    gren.getKDoc(),
